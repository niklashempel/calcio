...
{
    /// <summary>
    /// Group matches by venue into today / upcoming / past buckets relative to referenceUtc.
    /// Assumes referenceUtc is in UTC.
    /// </summary>
    public static IEnumerable<GroupedMatchesByVenueDto> GroupByVenue(IEnumerable<MatchDto> matches, DateTime referenceUtc)
    {
        ...
        foreach (var match in list)
        {
            ...
            if (match.Time.HasValue)
            {
                ...
                {
                    groupedMatch.Today.Add(match);
                }
                else if (time >= todayEnd)
                {
                    groupedMatch.Upcoming.Add(match);
                }
                else
                {
        ...
        {
            g.Today = g.Today.OrderBy(m => m.Time).ToList(); // ascending
