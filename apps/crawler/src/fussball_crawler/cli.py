...
def find_clubs_command(args: argparse.Namespace) -> int:
    ...
    for postal_code in postal_codes:
        ...
        if not validate_postal_code(postal_code):
            logger.error(f"Invalid postal code format: {postal_code}. Expected 5 digits (e.g., 01099)")
            total_errors += 1
            continue
    ...
    logger.info(f"Completed processing {total_processed} postal codes ({total_errors} errors)")
